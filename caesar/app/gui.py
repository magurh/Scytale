...
class CipherApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Substitution Cipher App")
        ...
        self.insert_key_button = tk.Button(root, text="Insert Key", command=self.toggle_insert_key)
        self.insert_key_button.grid(row=0, column=2)

        self.generate_key_button = tk.Button(root, text="Generate Random Key", command=self.toggle_generate_key)
        ...
        # Buttons for Actions
        self.encrypt_button = tk.Button(root, text="Encrypt", command=self.encrypt_text, bg="red")
        self.encrypt_button.grid(row=3, column=0, sticky="ew")

        self.decrypt_button = tk.Button(root, text="Decrypt", command=self.decrypt_text, bg="green")
        self.decrypt_button.grid(row=3, column=1, sticky="ew")

        self.freq_analysis_button = tk.Button(root, text="Decrypt by Frequency", command=self.freq_analysis, bg="green")
    ...
    def encrypt_text(self):
        ...
        self.decrypt_button.config(relief=tk.RAISED, bg="green")
        self.freq_analysis_button.config(relief=tk.RAISED, bg="green")
    ...
    def decrypt_text(self):
        self.encrypt_button.config(relief=tk.RAISED, bg="red")
